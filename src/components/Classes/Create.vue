...
const handleSubmit = async ()=>{
  ...
  try {
    errorMessage.value = "";
    await axios
      .post("http://localhost:3005/api/cours/", formData, {
        headers: {
...
<template>
  <admin-layout>
    <div class="flex items-center justify-between">
      <h1 class="text-black text-4xl text-left py-4 px-2">Ajouter une Classe</h1>
      <button
      ...
      >
        <router-link :to="`/classes`" class="flex items-center gap-x-2">
          <svg
            width="20"
            height="20"
            viewBox="0 0 24 24"
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white">
      ...
      >
        ...
        <!-- error messages -->
        <div v-if="errorMessage"><span class="text-red-600">{{ message }}</span></div>
        <!-- submit button -->
