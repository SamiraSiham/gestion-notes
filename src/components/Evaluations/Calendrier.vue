...
const getData = async () => {
  ...
  try {
    errorMessage.value = "";
    await axios
      .get("http://localhost:3005/api/evaluations/", {
        headers: {
      ...
      .then((res) => {
        ...
        evals.value = res.data.evaluations;
        // cours.value = res.data?.cours;
        
        
        evals.value.forEach((e: any) => {
          calendarData.push({
            id: e._id,
            title: e.titre,
            start: new Date(e.date_eval).toISOString().split("T")[0],
            extendedProps: { calendar: calendarsEvents[e.type] },
          });
        });
...
const calendarsEvents: any = reactive({
  Sommative : 'Sommative',
  Formative: 'Formative',
...
const handleAddOrUpdateEvent = () => {
  if (selectedEvent.value) {
    ...
    events.value = events.value.map((event: any) =>
...
const renderEventContent = (eventInfo: any) => {
  let col = eventInfo.event.extendedProps.calendar.toLowerCase();
  const colorClass =  col == 'diagnostique' ? 'fc-bg-primary' : col == 'sommative' ? 'fc-bg-danger' : col == 'formative' ? 'fc-bg-success' : 'fc-bg-gray-300'
  ;
  return {
    html: `
